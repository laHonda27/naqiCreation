...
import { useCreations, Creation, OrderProcess, Specifications } from '../../hooks/useCreations';

const CreationsManager: React.FC = () => {
  const { creations, categories, addCreation, updateCreation, deleteCreation } = useCreations();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      await updateCreation(editingId, cleanedFormData);
      setEditingId(null);
    } else {
      // Add new creation
      await addCreation(cleanedFormData);
    }
      ...
      {isAdding ? (
        <div className="bg-beige-50 p-6 rounded-lg border border-beige-200 mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {formData.technicalDetails.map((detail, index) => (
                      <div key={index} className="flex items-center mb-2">
                        <input
                          ...
                          className="input-field text-sm flex-grow"
                          placeholder="Entrez un détail technique"
                        />
                        <button
                          type="button"
            ...
            {/* Section 4: Options de personnalisation */}
            <div className="bg-white p-4 rounded-lg border border-beige-200">
              <button 
                type="button" 
                className="flex justify-between w-full items-center text-left font-medium text-taupe-800 mb-2"
              ...
              >
                ...
                {activeSection === 'options' ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
              </button>
                    ...
                    {formData.customizationOptions.map((option, index) => (
                      <div key={index} className="flex items-center mb-2">
                        <input
                          ...
                          className="input-field text-sm flex-grow"
                          placeholder="Entrez une option de personnalisation"
                        />
                        <button
                          type="button"
            ...
            {/* Section 5: Processus de commande */}
            <div className="bg-white p-4 rounded-lg border border-beige-200">
              <button 
                type="button" 
                className="flex justify-between w-full items-center text-left font-medium text-taupe-800 mb-2"
              ...
              >
                ...
                {activeSection === 'process' ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
              </button>
              
              {activeSection === 'process' && (
                <div className="pt-2 space-y-4">
                  {formData.orderProcess.map((step, index) => (
                    <div key={index} className="border border-beige-200 p-3 rounded-md bg-beige-50">
